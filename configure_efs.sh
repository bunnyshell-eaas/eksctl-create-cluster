...
export FILE_SYSTEM_ID=$(aws efs create-file-system --output json | jq --raw-output '.FileSystemId')
...
aws efs describe-file-systems --file-system-id $FILE_SYSTEM_ID --output text
...
subnets=($(aws ec2 describe-subnets --filters "Name=$TAG1,Values=$EKS_CLUSTER_NAME" "Name=$TAG2,Values=1" --output json | jq --raw-output '.Subnets[].SubnetId'))
...
while true; do
    ...
    output=$(aws efs describe-mount-targets --file-system-id $FILE_SYSTEM_ID --output json | jq --raw-output '.MountTargets[].LifeCycleState')
